...
void tower(void)
{
    ...
    while(t)
    {
        ...
        v0.x = g.enemy.monster[0].center.x - t->pos.x;
        v0.y = g.enemy.monster[0].center.y - t->pos.y;
        dist0 = sqrt(pow(v0.x, 2) + pow(v0.y, 2));
        if(dist0 < 200 && t->location == g.enemy.monster[0].location)
        {
            v0 = vecNormalize(v0);
...
void render(void)
{
    ...
    {
        setColor3i(0+40*(5-g.health), 0, 0);
        XFillRectangle(dpy, backBuffer, gc, 0, 0, g.xres, g.yres);
