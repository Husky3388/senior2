...
struct Global {
    ...
    int flashlight_point_left[2],
        flashlight_point_right[2];
    ...
    Global() {
        ...
        for(int i = 0; i < 4; i++)
        {
            enemy.monster[i].radius = circle.radius;
        }
        //spawnRate = 3;
...
void enemyMovement(Shape player, Shape enemy, int num)   // only works one enemy at a time, CHANGE LATER
{
    // check enemy location
    // check surrounding open spaces
    // decide which direction to go

    if(g.enemy.spawn[num] == true && num == 0) // 1st enemy
    {
        ...
        if(distance(player.center.x, enemy.center.x, player.center.y, enemy.center.y - enemy.radius*2) < dist) // check whether going up places enemy closer to player
        {
            ...
            {
                g.enemy.monster[num].center.y -= g.enemy.monster[num].radius*2;
            }
            else
            {
                g.health--;
        ...
        if(distance(player.center.x, enemy.center.x - enemy.radius*2, player.center.y, enemy.center.y) < dist) // left
        {
            ...
            {
                g.enemy.monster[num].center.x -= g.enemy.monster[num].radius*2;
            }
            else
            {
                g.health--;
    ...
    if(g.enemy.spawn[1] == true && num == 1) // 2nd enemy
    {
        // make enemy go around border?  if in flashlight, move 2 spaces, chase player
        // if enemy is in flashlight, chase player 2 spaces, similar algorithm to 1st enemy
        // check if inside the flashlight on all 4 sides of player (make flashlight triangle slightly bigger) *************************************
        if((isInside(g.circle.center.x, g.circle.center.y, 
                        g.flashlight_point_left[0], g.flashlight_point_left[1],
                        g.flashlight_point_right[0], g.flashlight_point_right[1],
                        enemy.center.x, enemy.center.y)) &&
                (isInside(g.circle.center.x, g.circle.center.y, 
                          g.flashlight_point_left[0], g.flashlight_point_left[1],
                          g.flashlight_point_right[0], g.flashlight_point_right[1],
                          enemy.center.x, enemy.center.y)) &&
                (isInside(g.circle.center.x, g.circle.center.y, 
                          ...
                          enemy.center.x, enemy.center.y)))
        {
            for(int i = 0; i < 2; i++)
            {
                ...
                if(distance(player.center.x, enemy.center.x, player.center.y, enemy.center.y - enemy.radius*2) < dist) // check whether going up places enemy closer to player
                {
                    ...
                    {
                        g.enemy.monster[num].center.y -= g.enemy.monster[num].radius*2;
                        enemy.center.y = g.enemy.monster[num].center.y;
                    }
                    else
                    {
                        g.health--;
                ...
                else if(distance(player.center.x, enemy.center.x - enemy.radius*2, player.center.y, enemy.center.y) < dist) // left
                {
                    if((enemy.center.x - enemy.radius*2 != player.center.x) || (enemy.center.y != player.center.y))
                    {
                        g.enemy.monster[num].center.x -= g.enemy.monster[num].radius*2;
                        enemy.center.x = g.enemy.monster[num].center.x;
                    }
                    else
                    {
                        g.health--;
                ...
                else if(distance(player.center.x, enemy.center.x, player.center.y, enemy.center.y + enemy.radius*2) < dist) // down
                {
                    if((enemy.center.x != player.center.x) || (enemy.center.y + enemy.radius*2 != player.center.y))
                    {
                        g.enemy.monster[num].center.y += g.enemy.monster[num].radius*2;
                ...
                else if(distance(player.center.x, enemy.center.x + enemy.radius*2, player.center.y, enemy.center.y) < dist) // right
                {
                    if((enemy.center.x + enemy.radius*2 != player.center.x) || (enemy.center.y != player.center.y))
                    {
                        g.enemy.monster[num].center.x += g.enemy.monster[num].radius*2;
        ...
        //check corners
        if((enemy.center.x == g.enemy.monster[num].radius) && (enemy.center.y == g.enemy.monster[num].radius)) // top left corner
        {
            if((enemy.center.y + enemy.radius*2 != player.center.y) || (enemy.center.x != player.center.x))
            {
                g.enemy.monster[num].center.y += g.enemy.monster[num].radius*2;
            }
            else
        ...
        else if((enemy.center.x == g.enemy.monster[num].radius) && (enemy.center.y == g.yres - g.enemy.monster[num].radius)) // bottom left corner
        {
            if((enemy.center.x + enemy.radius*2 != player.center.x) || (enemy.center.y != player.center.y))
            {
                g.enemy.monster[num].center.x += g.enemy.monster[num].radius*2;
        ...
        else if((enemy.center.x == g.xres - g.enemy.monster[num].radius) && (enemy.center.y == g.yres - g.enemy.monster[num].radius)) // bottom right corner
        {
            if((enemy.center.y - enemy.radius*2 != player.center.y) || (enemy.center.x != player.center.x))
            {
                g.enemy.monster[num].center.y -= g.enemy.monster[num].radius*2;
        ...
        else if((enemy.center.x == g.xres - g.enemy.monster[num].radius) && (enemy.center.y == g.enemy.monster[num].radius)) // top right corner
        {
            if((enemy.center.x - enemy.radius*2 != player.center.x) || (enemy.center.y != player.center.y))
            {
                g.enemy.monster[num].center.x -= g.enemy.monster[num].radius*2;
        ...
        // now check where enemy should go
        if(isInside(g.xres/2, g.yres/2, 0, 0, g.xres, 0, enemy.center.x + 1, enemy.center.y) &&
                (isInside(g.xres/2, g.yres/2, 0, 0, g.xres, 0, enemy.center.x - 1, enemy.center.y))) // enemy is above center of map, move left
        {
            if((enemy.center.x - enemy.radius*2 != player.center.x) || (enemy.center.y != player.center.y))
            {
                g.enemy.monster[num].center.x -= g.enemy.monster[num].radius*2;
        ...
        else if(isInside(g.xres/2, g.yres/2, 0, 0, 0, g.yres, enemy.center.x, enemy.center.y + 1) &&
                (isInside(g.xres/2, g.yres/2, 0, 0, 0, g.yres, enemy.center.x, enemy.center.y - 1))) // enemy is to the left of center of map, move down
        {
            if((enemy.center.y - enemy.radius*2 != player.center.y) || (enemy.center.x != player.center.x))
            {
                g.enemy.monster[num].center.y += g.enemy.monster[num].radius*2;
        ...
        else if(isInside(g.xres/2, g.yres/2, 0, g.yres, g.xres, g.yres, enemy.center.x + 1, enemy.center.y) &&
                (isInside(g.xres/2, g.yres/2, 0, g.yres, g.xres, g.yres, enemy.center.x - 1, enemy.center.y))) // enemy is below center of map, move right
        {
            if((enemy.center.x + enemy.radius*2 != player.center.x) || (enemy.center.y != player.center.y))
            {
                g.enemy.monster[num].center.x += g.enemy.monster[num].radius*2;
        ...
        else if(isInside(g.xres/2, g.yres/2, g.xres, 0, g.xres, g.yres, enemy.center.x, enemy.center.y + 1) &&
                (isInside(g.xres/2, g.yres/2, g.xres, 0, g.xres, g.yres, enemy.center.x, enemy.center.y - 1))) // enemy is to the right of center of map, move up
        {
            if((enemy.center.y - enemy.radius*2 != player.center.y) || (enemy.center.x != player.center.x))
            {
                g.enemy.monster[num].center.y -= g.enemy.monster[num].radius*2;
...
void nextTurn()
{
    for(int i = 0; i < 4; i++)
    {
        enemyMovement(g.circle, g.enemy.monster[i], i); 
    }
    tower();
    g.maps.turns--;
    if(g.maps.turns == 0)
    ...
    for(int i = 0; i < 2; i++)
    {
        if(g.enemy.spawn[i] == true && g.enemy.monster[i].location != g.maps.location && g.enemy.spawnRate[i] == 0)
        {
            deleteEnemy(i);
        }
        if(g.enemy.spawn[i] == false)
    ...
    else if(!g.title && !g.showOptions && !g.showRes && !g.showMaps) // game
    {
        switch (key) {
            ...
            case XK_w:
                ...
                //g.direction = 0;
                //for(int i = 0; i < 4; i++) // don't move if any enemy is in front *********************************************************
                //{
                if(g.direction == 0 && (g.circle.center.y - g.circle.radius*2) < 0 && g.maps.area[g.maps.location][0] > -1)
                {
                    ...
                    g.maps.location = g.maps.area[g.maps.location][0];
                        ...
                        ((g.enemy.monster[0].center.x != g.circle.center.x) || (g.enemy.monster[0].center.y + g.enemy.monster[0].radius*2 != g.circle.center.y)))
                    g.circle.center.y -= g.circle.radius*2;
                ...
                if(g.direction == 1 && (g.circle.center.x - g.circle.radius*2) < 0 && g.maps.area[g.maps.location][1] > -1)
                {
                    ...
                    g.maps.location = g.maps.area[g.maps.location][1];
                        ...
                        ((g.enemy.monster[0].center.x + g.enemy.monster[0].radius*2 != g.circle.center.x) || g.enemy.monster[0].center.y != g.circle.center.y))
...
void tower(void)
{
    ...
    while(t)
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            v0.x = g.enemy.monster[i].center.x - t->pos.x;
            v0.y = g.enemy.monster[i].center.y - t->pos.y;
            dist0 = sqrt(pow(v0.x, 2) + pow(v0.y, 2));
            if(dist0 < 200 && t->location == g.enemy.monster[i].location)
            {
                v0 = vecNormalize(v0);
                shootBullet(t->pos.x, t->pos.y, v0.x*g.circle.radius, v0.y*g.circle.radius);
                t->ammo--;
            }
        }
...
void spawnEnemy(int x)
{
    ...
    if(x == 0 || x == 1) // change later to < 4
    {
        if(rb == 0)     // spawn top
        {
            g.enemy.monster[x].center.x = rx * g.grid.size + g.circle.radius;
            g.enemy.monster[x].center.y = g.circle.radius;
        }
        else if(rb == 1) // spawn left
        {
            g.enemy.monster[x].center.x = g.circle.radius;
            g.enemy.monster[x].center.y = ry * g.grid.size + g.circle.radius;
        }
        else if(rb == 2) // spawn bottom
        {
            g.enemy.monster[x].center.x = rx * g.grid.size + g.circle.radius;
            g.enemy.monster[x].center.y = g.yres - g.circle.radius;
        }
        else if(rb == 3) // spawn right
        {
            g.enemy.monster[x].center.x = g.xres - g.circle.radius;
            g.enemy.monster[x].center.y = ry * g.grid.size + g.circle.radius;
        }
        g.enemy.monster[x].location = g.maps.location;
        g.enemy.spawn[x] = true;
    }
    /*else if(x == 1)
      {
      if(rb == 0)     // spawn top
...
void deleteEnemy(int x)
{
    if(x == 0 || x == 1) // change to < 4
    {
        g.enemy.monster[x].center.x = -g.circle.radius;
        g.enemy.monster[x].center.y = -g.circle.radius;
        g.enemy.monster[x].location = -1;
        //delete g.enemy.monster[0];
        //g.enemy.monster[0] == NULL;
        g.enemy.spawn[x] = false;
        if(x == 0)
            g.enemy.spawnRate[x] = 3;
        else if(x == 1)
            g.enemy.spawnRate[x] = 6;
    }
}
...
void physics(void)
{
    ...
    while(b)
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            if(b->pos.x > g.enemy.monster[i].center.x - g.circle.radius && b->pos.x < g.enemy.monster[i].center.x + g.circle.radius &&
                    b->pos.y > g.enemy.monster[i].center.y - g.circle.radius && b->pos.y < g.enemy.monster[i].center.y + g.circle.radius)
            {
                deleteBullet(b);
                deleteEnemy(i);
                //b = b->next;
...
void render(void)
{
    ...
    {
        setColor3i(50+40*(5-g.health), 50, 50);
        XFillRectangle(dpy, backBuffer, gc, 0, 0, g.xres, g.yres);
        ...
        if(g.direction == 0)
        {
            if(g.flashlight)
            {
                g.flashlight_point_left[0] = g.circle.center.x - g.circle.radius*9;
                g.flashlight_point_left[1] = g.circle.center.y - g.circle.radius*9;
                g.flashlight_point_right[0] = g.circle.center.x + g.circle.radius*9;
                g.flashlight_point_right[1] = g.circle.center.y - g.circle.radius*9;
            }
            else
            {
                g.flashlight_point_left[0] = g.circle.center.x - g.circle.radius*5;
                g.flashlight_point_left[1] = g.circle.center.y - g.circle.radius*5;
                g.flashlight_point_right[0] = g.circle.center.x + g.circle.radius*5;
                g.flashlight_point_right[1] = g.circle.center.y - g.circle.radius*5;
            }   
        }
        if(g.direction == 1)
        {
        ...
        if(g.maps.location == g.enemy.monster[1].location)
        {
            setColor3i(255,255,255);
            XDrawArc(dpy, backBuffer, gc, g.enemy.monster[1].center.x-g.circle.radius, g.enemy.monster[1].center.y-g.circle.radius,
                    g.circle.radius*2, g.circle.radius*2, 0, 360*64);
            setColor3i(255,255,0);
            XFillArc(dpy, backBuffer, gc, g.enemy.monster[1].center.x-g.circle.radius, g.enemy.monster[1].center.y-g.circle.radius,
                    g.circle.radius*2, g.circle.radius*2, 0, 360*64);
        }
        ...
        // render layers, flashlight/darkness
        if(!g.drawGrid)
        {
            setColor3i(0, 0, 0);
                ...
                for(int j = 0; j < g.yres; j++)
                {
                                ...
                                i, j))
                    {
                        /*XColor c;
                          ...
                          XImage *image;
                          image = XGetImage(dpy, RootWindow(dpy, DefaultScreen(dpy)), i, j, 1, 1, AllPlanes, XYPixmap);
                          c.pixel = XGetPixel(image, 0, 0);
                          XFree(image);
                          XQueryColor(d, DefaultColormap(dpy, DefaultScreen(dpy)), &c);
                          cout << c.red/256 << " " << c.green/256 << " " << c.blue/256 << "\n";*/

                        //glReadPixels(i, j, 1, 1, GL_RGB, GL_UNSIGNED_BYTE, color);
                        //setColor3i((int)color[0]/255, (int)color[1]/255, (int)color[2]/255);
                        //cout << (int)color[0]/255 << " " << (int)color[1]/255 << " " << (int)color[2]/255 << endl;
                        XDrawPoint(dpy, backBuffer, gc, i, j);
                    }
                }
            }
        }
